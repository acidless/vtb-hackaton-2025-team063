        ...
        <motion.div className="flex items-center justify-between" initial={{opacity: 0, y: 10}}
            ...
            <div className="flex items-center gap-2 min-w-0">
                <div className="flex flex-row-reverse items-center">
                    <TransactionCategoryAvatar className="-ml-6" categoryId={cashback.category}/>
                    <Avatar avatar={getAbsoluteSeverUrl(userAvatar)} size="3.125"/>
                </div>
                <div className="flex flex-col min-w-0">
                    <p className="font-medium text-ellipsis overflow-hidden whitespace-nowrap">{TransactionCategories[cashback.category].name}</p>
            ...
            <div className="shrink-0 flex flex-col items-end">
                <p className="text-info text-2xl font-bold">{cashback.percents}%</p>
                <p className="text-light text-xs">Действует до <time>{cashback.date.toLocaleDateString()}</time></p>
            </div>
        </motion.div>
    </motion.article>
}