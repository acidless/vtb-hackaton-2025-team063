...
export const InvitePartner = ({codeData, isLoading, isActive, setActive}: Props) => {
    ...
    const codeStr = useMemo(() => {
        ...
        return codeData.code.slice(0, 3) + " " + codeData.code.slice(3);
    }, [codeData]);
    ...
    useEffect(() => {
        ...
        interval.current = setInterval(() => {
            setValidFor(diffTime(new Date(), new Date(codeData.expiresAt)));
        }, 1000);
    ...
    return <ModalWindow isActive={isActive} setActive={setActive}>
        ...
        <div className="mb-2.5">
            <AccentButton onClick={handleCopy}
                          className="w-full justify-center gap-1 py-2 text-xs">Скопировать</AccentButton>
        </div>
        <InfoPopup className="-top-15!" text="Код успешно скопирован!" time={2000} isActive={isCopied}
                   setActive={setCopied}/>
    </ModalWindow>
}