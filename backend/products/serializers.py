...
class ProductSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Product
        fields = [
            'id', 'bank', 'bank_name', 'product_id', 'product_type', 'name',
            'description', 'short_description', 'interest_rate', 'min_amount',
            'max_amount', 'term_months', 'currency', 'features', 'requirements',
            'documents_required', 'status', 'is_featured', 'promotion_end',
            'category_name', 'is_promotional', 'created_at', 'updated_at'
        ]
    
    def get_category_name(self, obj):
        # Логика определения категории на основе типа продукта
        category_map = {
            'DEPOSIT': 'Вклады',
            'LOAN': 'Кредиты',
            'CARD': 'Карты',
    ...
    def get_is_promotional(self, obj):
        ...
        return obj.promotion_end and obj.promotion_end > timezone.now()
...
class ProductAgreementSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'id', 'user_profile', 'team_id', 'product', 'product_name', 'bank_name',
            'agreement_id', 'status', 'amount', 'interest_rate', 'term_months',
            'opened_date', 'closed_date', 'maturity_date', 'linked_account',
            'current_balance', 'interest_accrued', 'next_payment_date',
            'next_payment_amount', 'days_until_maturity', 'created_at', 'updated_at'
        ]
...
class ProductApplicationSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'id', 'user_profile', 'team_id', 'product', 'product_name', 'bank_name',
            'application_id', 'status', 'requested_amount', 'requested_term',
            'purpose', 'application_data', 'approved_amount', 'approved_term',
            'approved_interest_rate', 'rejection_reason', 'submitted_at',
...
class ProductOfferSerializer(serializers.Serializer):
    """Сериализатор для персональных предложений"""
    product_id = serializers.IntegerField()
