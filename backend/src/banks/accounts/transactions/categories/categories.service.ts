...
import {TransformedTransaction} from "../transaction.transformer";
...
export class CategoriesService {
    ...
    public async getCategories(userId: number) {
        return this.redisService.withCache(`${this.baseKey}:${userId}`, 300, async () => {
            ...
            return this.groupTransactionsByCategories(transactions);
        });
    }

    public async groupTransactionsByCategories(transactions: TransformedTransaction[]) {
