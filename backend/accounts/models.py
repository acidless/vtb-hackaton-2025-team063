...
class Account(models.Model):
    ...
    account_id = models.CharField(max_length=100, db_index=True)  # ID из банковского API
    ...
    iban = models.CharField(max_length=34, blank=True)  # Международный номер счета
    opened_date = models.DateTimeField(default=timezone.now)
    closed_date = models.DateTimeField(null=True, blank=True)
    last_sync = models.DateTimeField(auto_now=True)
    raw_data = JSONField(default=dict)  # Полные данные из API банка
    ...
    def __str__(self):
        return f"{self.user_profile.team_id} - {self.account_id} ({self.get_account_type_display()})"
...
class Transaction(models.Model):
    ...
    account = models.ForeignKey(Account, on_delete=models.CASCADE, db_index=True, related_name='transactions')
    transaction_id = models.CharField(max_length=100, db_index=True)
    bank_transaction_id = models.CharField(max_length=100, blank=True)  # ID транзакции в банке
    amount = models.DecimalField(max_digits=15, decimal_places=2)
    currency = models.CharField(max_length=3, default='RUB')
    type = models.CharField(max_length=10, choices=TRANSACTION_TYPES)
    status = models.CharField(max_length=20, choices=TRANSACTION_STATUS, default='BOOKED')
    ...
    transaction_code = models.CharField(max_length=50, blank=True)  # Код операции
    reference = models.CharField(max_length=255, blank=True)  # Референс операции
    ...
    def __str__(self):
        return f"{self.account.account_id} - {self.amount} {self.currency} - {self.description}"


class Balance(models.Model):
    account = models.ForeignKey(Account, on_delete=models.CASCADE, db_index=True, related_name='balances')
    balance_type = models.CharField(max_length=50, default='CLOSING_BOOKED')
    amount = models.DecimalField(max_digits=15, decimal_places=2)
    currency = models.CharField(max_length=3, default='RUB')
    credit_limit = models.DecimalField(max_digits=15, decimal_places=2, null=True, blank=True)
    credit_used = models.DecimalField(max_digits=15, decimal_places=2, null=True, blank=True)
    last_updated = models.DateTimeField(auto_now=True)

    class Meta:
        db_table = 'balances'
        indexes = [
            models.Index(fields=['account', 'last_updated']),
        ]
...
class AccountSyncLog(models.Model):
    ...
    sync_type = models.CharField(max_length=20, choices=[('FULL', 'Полная'), ('DELTA', 'Дельта')])
