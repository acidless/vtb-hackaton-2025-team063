...
class Payment(models.Model):
    ...
    payment_id = models.CharField(max_length=100, db_index=True)  # ID из банковского API
    consent_id = models.CharField(max_length=100, blank=True)
    payment_type = models.CharField(max_length=20, choices=PAYMENT_TYPES, default='INTERNAL')
...
class PaymentConsent(models.Model):
    ...
    # Параметры согласия
    debtor_account = models.CharField(max_length=100)
    max_amount_per_payment = models.DecimalField(max_digits=15, decimal_places=2, null=True, blank=True)
    max_total_amount = models.DecimalField(max_digits=15, decimal_places=2, null=True, blank=True)
    max_uses = models.IntegerField(null=True, blank=True)
    used_count = models.IntegerField(default=0)
    used_amount = models.DecimalField(max_digits=15, decimal_places=2, default=0)
    
    # Для VRP (Variable Recurring Payments)
    vrp_max_individual_amount = models.DecimalField(max_digits=15, decimal_places=2, null=True, blank=True)
    vrp_daily_limit = models.DecimalField(max_digits=15, decimal_places=2, null=True, blank=True)
    vrp_monthly_limit = models.DecimalField(max_digits=15, decimal_places=2, null=True, blank=True)
    ...
    def can_be_used(self, amount):
        """Проверка возможности использования согласия"""
        if self.status != 'AUTHORISED' or self.is_expired():
            return False
        
        if self.max_amount_per_payment and amount > self.max_amount_per_payment:
            return False
        
        if self.max_total_amount and (self.used_amount + amount) > self.max_total_amount:
            return False
...
class PaymentLimit(models.Model):
    """Лимиты на платежи пользователя"""
    user_profile = models.OneToOneField(UserBankProfile, on_delete=models.CASCADE, db_index=True)
    ...
    monthly_limit = models.DecimalField(max_digits=15, decimal_places=2, default=2000000)
    per_transaction_limit = models.DecimalField(max_digits=15, decimal_places=2, default=50000)
    ...
    def reset_if_needed(self):
        """Сброс использованных лимитов при необходимости"""
        ...
        # Сброс недельного лимита в понедельник
        if today.weekday() == 0 and today > self.last_reset_weekly:
            self.weekly_used = 0
            self.last_reset_weekly = today
        
        # Сброс месячного лимита в первый день месяца
