...
from .serializers import (
    ...
    PaymentConsentSerializer, PaymentConsentCreateSerializer,
...
class PaymentConsentViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        user_profile = get_object_or_404(UserBankProfile, user=self.request.user)
        return PaymentConsent.objects.filter(user_profile=user_profile).select_related('bank')
    ...
    def create(self, request):
        ...
        if serializer.is_valid():
            ...
            try:
                ...
                return Response(
                    PaymentConsentSerializer(consent).data,
                    status=status.HTTP_201_CREATED
                )
            except Exception as e:
                return Response(
    ...
    def revoke(self, request, pk=None):
        ...
        # Логика отзыва согласия через API банка
        # ...
...
class PaymentLimitViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def reset(self, request, pk=None):
        """Принудительный сброс лимитов (для тестирования)"""
